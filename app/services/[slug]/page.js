            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {others.map((s) => (
                <Link
                  key={s.id}
                  href={`/services/${s.id}`}
                  className="rounded-full bg-white/5 px-5 py-2.5 text-sm font-semibold text-white/70 transition-all hover:-translate-y-0.5 hover:bg-white/10 hover:text-white"
                >
                  {s.title}
                </Link>
              ))}
