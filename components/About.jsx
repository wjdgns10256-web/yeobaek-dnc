    ...
    <section id="about" className="bg-white py-20 sm:py-28">
      <div className="section-pad mx-auto max-w-content">
        <div className="max-w-2xl">
          ...
          <div className="mt-8 flex flex-wrap gap-x-10 gap-y-4 border-t border-ink-100 pt-6">
            <div>
              <p className="text-2xl font-extrabold tracking-tight text-ink">
                {siteConfig.trackRecord.projects}
              </p>
              <p className="mt-1 text-xs tracking-wide text-ink-500">참여 현장</p>
            </div>
            <div>
              <p className="text-2xl font-extrabold tracking-tight text-ink">
                {siteConfig.trackRecord.since}
              </p>
              <p className="mt-1 text-xs tracking-wide text-ink-500">현장 경력 시작</p>
            </div>
            <div>
              <p className="text-2xl font-extrabold tracking-tight text-ink">{siteConfig.founded}</p>
              <p className="mt-1 text-xs tracking-wide text-ink-500">법인 설립</p>
            </div>
          </div>
          <p className="mt-2 text-xs text-ink-400">* {siteConfig.trackRecord.note}</p>
        </div>
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {values.map((value) => (
            <div
              key={value.title}
              className="group rounded-2xl border border-ink-100 bg-white p-7 shadow-sm transition-all hover:-translate-y-1 hover:border-accent-200 hover:shadow-lg"
            >
